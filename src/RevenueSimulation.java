...
public class RevenueSimulation {
    ...
    public void removeFilmFromScreenings(Film film) {
        boolean removed = screenings.remove(film);  // Try to remove the film from the queue
    
        if (removed) {
            System.out.println("Film '" + film.getFilmName() + "' has been removed from screenings.");
        } else {
